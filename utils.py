...
def input_setup_demo(args, mode):
    ...
    if mode == "train":
        ...
        for i in range(len(data)):
            ...
            h, w, _ = label_.shape  # only for R,G,B image
            for x in range(0, h - args.patch_size*args.scale + 1, args.patch_size*args.scale):
                for y in range(0, w - args.patch_size*args.scale + 1, args.patch_size*args.scale):
                    sub_label = label_[x:x + args.patch_size*args.scale, y:y + args.patch_size*args.scale, :]
                    sub_label_sequence.append(sub_label)
